...
class Snaggable(val session: SnagSession,
  ...
  def toGenerated(): Snaggable = {
    val genGav = gav.toSnapshot()
    new Snaggable(session, genGav, jar, Snaggable.genPom(session, genGav))
  }
}
...
object Snaggable {
   ...
   */
  def genPom(session: SnagSession, gav: GAV): File = {
